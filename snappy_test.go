...
var snappyStreamTestCases = map[string][]byte{
	...
	"A":                                 {0x82, 0x53, 0x4e, 0x41, 0x50, 0x50, 0x59, 0x0, 0x0, 0x0, 0x0, 0x1, 0x0, 0x0, 0x0, 0x1, 0x0, 0x0, 0x0, 0x3, 0x1, 0x0, 0x41},
	"":                                  {0x82, 0x53, 0x4e, 0x41, 0x50, 0x50, 0x59, 0x0, 0x0, 0x0, 0x0, 0x1, 0x0, 0x0, 0x0, 0x1},
}
...
func BenchmarkSnappyStreamEncodeMassive(b *testing.B) {
	...
	var (
		dst = make([]byte, 0, snappy.MaxEncodedLen(len(massiveString)))
		err error
	)
	...
	for n := 0; n < b.N; n++ {
		dst = EncodeStream(dst[:0], massiveString)
		if err != nil {
...
func BenchmarkSnappyStreamEncode(b *testing.B) {
	...
	var (
		dst = make([]byte, 0, snappy.MaxEncodedLen(len(test)))
		err error
	)
